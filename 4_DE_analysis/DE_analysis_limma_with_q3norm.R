...
ggplot(voomplot) + 
  geom_point(mapping = aes(x = avgs, y = root_sds)) + 
  geom_smooth(mapping = aes(x = avgs, y = root_sds), se = F, color = "red") + 
  ggthemes::theme_base()

pdf(file = "comparisons_first_cohort_volcanos_limma_with_q3norm.pdf", width = 8, height = 8)
for (c in names(contrast_list)) {
  ...
  write.csv(deout, paste(gsub(pattern = "/", replacement = "vs", x = c), " limma with q3norm.csv", sep = ""))
  
  deout <- dplyr::rename(deout, fdr = adj.P.Val, coefficient = logFC, pvalue = P.Value)
...
pdf(file = "comparisons_first_cohort_heatmaps_limma_with_q3norm.pdf", width = 25, height = 50)
...
for (c in names(contrast_list)) {
  restest <- read.csv(paste(gsub(pattern = "/", replacement = "vs", x = c), " limma with q3norm.csv", sep = ""), row.names = 1)
  restest <- dplyr::rename(restest, fdr = adj.P.Val, coefficient = logFC, pvalue = P.Value)
...
pdf(file = "comparisons_second_cohort_volcanos_limma_with_q3norm.pdf", width = 8, height = 8)
for (c in names(contrast_list)) {
  deout <- contrasts.fit(limmafit, contrasts = contrast_list[[c]])
  deout <- eBayes(deout) |> topTable(number = Inf)
  deout$target <- rownames(deout)
...
pdf(file = "comparisons_second_cohort_heatmaps_limma_with_q3norm.pdf", width = 25, height = 50)
